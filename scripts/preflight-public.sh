...
echo "== Sentinel public release preflight =="
...
if pnpm run build >/tmp/sentinel-preflight-build.log 2>&1; then
  pass "Build succeeded"
else
  fail "Build failed (see /tmp/sentinel-preflight-build.log)"
  tail -n 80 /tmp/sentinel-preflight-build.log || true
fi
