...
usage() {
  cat <<'EOF'
Sentinel one-click setup (macOS/Linux)

Usage:
  bash scripts/opensentinel-setup.sh [--agents-path PATH] [--port PORT] [--opensentinel-config PATH] [--start]

Options:
  --agents-path PATH       Custom AGENTS.md path for pnpm setup
  --port PORT              Override PORT in .env and AGENTS template injection
  --opensentinel-config PATH   Path to opensentinel.json (default: ~/.opensentinel/opensentinel.json when present)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --port)
      ...
      ;;
    --opensentinel-config)
      [[ $# -ge 2 ]] || { echo "Missing value for --opensentinel-config" >&2; exit 1; }
      OPENCLAW_CONFIG="$2"
      shift 2
      ;;
    --start)
...
if [[ ! -f package.json || ! -f scripts/setup.mjs ]]; then
  echo "Run this script from the Sentinel repository." >&2
  exit 1
fi
...
if [[ -z "${OPENCLAW_CONFIG}" ]]; then
  DEFAULT_OPENCLAW_CONFIG="$(expand_path "~/.opensentinel/opensentinel.json")"
  ...
  if [[ ! -f "${OPENCLAW_CONFIG}" ]]; then
    echo "[Sentinel] Warning: OPENCLAW config not found at ${OPENCLAW_CONFIG}. Keeping path for later."
  fi
fi
...
if (!currentSecret || currentSecret === "__CHANGE_ME__") {
  ...
  console.log("[Sentinel] Generated OAUTH_ENCRYPTION_SECRET");
}
...
if (!currentInboxSecret || currentInboxSecret === "__CHANGE_ME__") {
  ...
  console.log("[Sentinel] Generated INBOX_WEBHOOK_SECRET");
}

const port = process.env.CLAW_SETUP_PORT?.trim();
if (port) {
  upsert("PORT", port);
  console.log(`[Sentinel] Set PORT=${port}`);
}

const opensentinel = process.env.CLAW_SETUP_OPENCLAW?.trim();
if (opensentinel) {
  const normalized = opensentinel.replace(/\\/g, "/");
  upsert("OPENCLAW_CONFIG", `"${normalized}"`);
  console.log(`[Sentinel] Set OPENCLAW_CONFIG=${normalized}`);
}
...
echo "[Sentinel] Installing AGENTS.md orchestration rules..."
...
if [[ "${START_AFTER_SETUP}" == "1" ]]; then
  echo "[Sentinel] Starting development server..."
  exec pnpm dev:local
fi
