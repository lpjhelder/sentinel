...
export function registerGracefulShutdownHandlers({
  ...
  function gracefulShutdown(signal: string): void {
    console.log(`\n[Sentinel] ${signal} received. Shutting down gracefully...`);
    ...
    for (const [taskId, child] of activeProcesses) {
      console.log(`[Sentinel] Stopping process for task ${taskId} (pid: ${child.pid})`);
    ...
    setTimeout(() => {
      console.error("[Sentinel] Forced exit after timeout.");
      process.exit(1);
    }, 5000).unref();
  }
