  ...
  async function withSqliteBusyRetry<T>(operation: string, fn: () => T): Promise<T> {
    for (let attempt = 0; attempt <= SQLITE_BUSY_RETRY_MAX_ATTEMPTS; attempt++) {
      try {
        const result = fn();
        if (attempt > 0) {
          console.warn(`[Claw-Republic] SQLite busy recovered: op=${operation}, retries=${attempt}`);
        }
        return result;
      } catch (err) {
        ...
        console.warn(
          `[Claw-Republic] SQLite busy: op=${operation}, attempt=${attempt + 1}/${SQLITE_BUSY_RETRY_MAX_ATTEMPTS + 1}, ` +
            `retry_in=${waitMs}ms`,
        );
