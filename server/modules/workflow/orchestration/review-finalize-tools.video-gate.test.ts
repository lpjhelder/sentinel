...
describe("review finalize video gate", () => {
  ...
  it("video artifact가 있어도 Remotion 증빙이 없으면 승인/머지를 차단한다", () => {
    const db = createDb();
    const projectRoot = fs.mkdtempSync(path.join(os.tmpdir(), "claw-republic-review-gate-"));
    const logsDir = fs.mkdtempSync(path.join(os.tmpdir(), "claw-republic-review-logs-"));
    try {
