...
export function createProjectContextTools(deps: CreateProjectContextToolsDeps) {
  ...
  function generateProjectContext(projectPath: string): string {
    const clawRepublicDir = path.join(projectPath, ".claw-republic");
    const contextPath = path.join(clawRepublicDir, "project-context.md");
    const metaPath = path.join(clawRepublicDir, "project-context.meta");
    ...
    if (isGitRepo(projectPath)) {
      try {
        ...
        fs.mkdirSync(clawRepublicDir, { recursive: true });
        fs.writeFileSync(contextPath, content, "utf8");
        fs.writeFileSync(metaPath, currentHead, "utf8");
        console.log(`[Claw-Republic] Generated project context: ${contextPath}`);
        return content;
      } catch (err) {
        console.warn(`[Claw-Republic] Failed to generate project context: ${err}`);
      }
    }
    ...
    if (isGitRepo(projectPath)) {
      ...
      try {
        ...
        for (const block of blocks) {
          const branchMatch = block.match(/branch refs\/heads\/(claw-republic\/[^\s]+)/);
  ...
  function ensureClaudeMd(projectPath: string, worktreePath: string): void {
    ...
    const clawRepublicDir = path.join(projectPath, ".claw-republic");
    const claudeMdSrc = path.join(clawRepublicDir, "CLAUDE.md");
    const claudeMdDst = path.join(worktreePath, "CLAUDE.md");

    const AUTO_GEN_MARKER = "This file was auto-generated by Claw Republic to provide project context.";
    ...
    if (shouldRefreshAutoGeneratedClaudeMd()) {
      ...
      console.log(`[Claw-Republic] Generated CLAUDE.md: ${claudeMdSrc}`);
    }

    try {
      fs.copyFileSync(claudeMdSrc, claudeMdDst);
    } catch (err) {
      console.warn(`[Claw-Republic] Failed to copy CLAUDE.md to worktree: ${err}`);
    }
  }
