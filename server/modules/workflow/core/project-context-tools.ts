...
export function createProjectContextTools(deps: CreateProjectContextToolsDeps) {
  ...
  function generateProjectContext(projectPath: string): string {
    ...
    const contextPath = path.join(sentinelDir, "project-context.md");
    const metaPath = path.join(sentinelDir, "project-context.meta");
    ...
    if (isGitRepo(projectPath)) {
      try {
        ...
        console.log(`[Sentinel] Generated project context: ${contextPath}`);
        return content;
      } catch (err) {
        console.warn(`[Sentinel] Failed to generate project context: ${err}`);
      }
    }
    ...
    if (isGitRepo(projectPath)) {
      ...
      try {
        ...
        for (const block of blocks) {
          const branchMatch = block.match(/branch refs\/heads\/(sentinel\/[^\s]+)/);
  ...
  function ensureClaudeMd(projectPath: string, worktreePath: string): void {
    ...
    const sentinelDir = path.join(projectPath, ".sentinel");
    const claudeMdSrc = path.join(sentinelDir, "CLAUDE.md");
    const claudeMdDst = path.join(worktreePath, "CLAUDE.md");

    const AUTO_GEN_MARKER = "This file was auto-generated by Sentinel to provide project context.";
    ...
    if (shouldRefreshAutoGeneratedClaudeMd()) {
      ...
      console.log(`[Sentinel] Generated CLAUDE.md: ${claudeMdSrc}`);
    }

    try {
      fs.copyFileSync(claudeMdSrc, claudeMdDst);
    } catch (err) {
      console.warn(`[Sentinel] Failed to copy CLAUDE.md to worktree: ${err}`);
    }
  }
