...
export function createWorktreeMergeTools(deps: CreateWorktreeMergeToolsDeps) {
  ...
  function mergeWorktree(
    ...
    try {
      ...
      const mergeMsg = `Merge sentinel task ${taskId.slice(0, 8)} (branch ${info.branchName})`;
    ...
    try {
      ...
      try {
        ...
        if (!devExists) {
          ...
          console.log(`[Sentinel] Created dev branch from main for task ${taskId.slice(0, 8)}`);
        }
      } catch {
        try {
        ...
        void (async () => {
          try {
            ...
            if (Array.isArray(existingPRs) && existingPRs.length > 0) {
              ...
              console.log(`[Sentinel] Existing PR updated: ${prUrl}`);
              ...
              if (createRes.ok) {
                ...
                console.log(`[Sentinel] Created PR: ${prData.html_url}`);
                appendTaskLog(taskId, "system", `GitHub PR created: ${prData.html_url}`);
              } else {
                const errBody = await createRes.text();
                console.warn(`[Sentinel] Failed to create PR: ${createRes.status} ${errBody}`);
                appendTaskLog(taskId, "system", `GitHub PR creation failed: ${createRes.status}`);
              }
            }
          } catch (prErr) {
            console.warn(`[Sentinel] PR creation error:`, prErr);
          }
        })();
      }
