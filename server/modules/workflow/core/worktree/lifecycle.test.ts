...
function initRepo(basePrefix: string): string {
  ...
  runGit(dir, ["config", "user.name", "Claw-Republic Test"]);
  runGit(dir, ["config", "user.email", "claw-republic-test@example.local"]);
...
describe("worktree lifecycle branch collision handling", () => {
  it("reuses existing task branch when branch already exists", () => {
    const repo = initRepo("claw-republic-wt-reuse-");
    tempDirs.push(repo);
    const shortId = "reuse001";
    const taskId = `${shortId}-0000-0000-0000-000000000000`;
    runGit(repo, ["branch", `claw-republic/${shortId}`]);
    ...
    expect(info?.branchName).toBe(`claw-republic/${shortId}`);
  ...
  it("falls back to suffixed branch when existing branch is occupied in another worktree", () => {
    const repo = initRepo("claw-republic-wt-fallback-");
    tempDirs.push(repo);
    const shortId = "fallback";
    const baseBranch = `claw-republic/${shortId}`;
