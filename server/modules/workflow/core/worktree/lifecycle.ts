...
export function createWorktreeLifecycleTools(deps: CreateWorktreeLifecycleToolsDeps) {
  ...
  function ensureWorktreeBootstrapRepo(projectPath: string, taskId: string): boolean {
    ...
    try {
      ...
      const baseIgnore = ["node_modules/", "dist/", ".sentinel-worktrees/", ".sentinel/", ".DS_Store", "*.log"];
      let existingExclude = "";
      try {
      ...
      if (!readConfig("user.name")) {
        execFileSync("git", ["config", "user.name", "Sentinel Bot"], {
          cwd: projectPath,
          stdio: "pipe",
          timeout: 3000,
        });
      ...
      if (!readConfig("user.email")) {
        execFileSync("git", ["config", "user.email", "sentinel@local"], {
          cwd: projectPath,
          stdio: "pipe",
          timeout: 3000,
        });
      ...
      const staged = execFileSync("git", ["diff", "--cached", "--name-only"], {
        ...
        .trim();
      if (staged) {
        execFileSync("git", ["commit", "-m", "chore: initialize project for Sentinel worktrees"], {
          cwd: projectPath,
          stdio: "pipe",
          timeout: 20000,
        });
      } else {
        execFileSync("git", ["commit", "--allow-empty", "-m", "chore: initialize project for Sentinel worktrees"], {
          cwd: projectPath,
          stdio: "pipe",
          timeout: 10000,
        });
      ...
      console.log(`[Sentinel] Auto-initialized git repo for task ${shortId} at ${projectPath}`);
      return true;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      appendTaskLog(taskId, "system", `Git bootstrap failed: ${msg}`);
      console.error(`[Sentinel] Failed git bootstrap for task ${shortId}: ${msg}`);
      return false;
    }
  }
  ...
  function createWorktree(projectPath: string, taskId: string, agentName: string, baseBranch?: string): string | null {
    ...
    const shortId = taskId.slice(0, 8);
    const branchName = `sentinel/${shortId}`;
    const worktreeBase = path.join(projectPath, ".sentinel-worktrees");
    ...
    try {
      ...
      console.log(
        `[Sentinel] Created worktree for task ${shortId}: ${selectedWorktreePath} (branch: ${selectedBranch}, agent: ${agentName})`,
      );
      return selectedWorktreePath;
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
      console.error(`[Sentinel] Failed to create worktree for task ${shortId}: ${msg}`);
      return null;
    }
  }
  ...
  function cleanupWorktree(projectPath: string, taskId: string): void {
    ...
    try {
      ...
      console.warn(`[Sentinel] git worktree remove failed for ${shortId}, falling back to manual cleanup`);
      try {
    ...
    try {
      ...
      console.warn(`[Sentinel] Failed to delete branch ${info.branchName} — may need manual cleanup`);
    }

    taskWorktrees.delete(taskId);
    console.log(`[Sentinel] Cleaned up worktree for task ${shortId}`);
  }
