export const DEFERRED_RUNTIME_FN_TAG = Symbol.for("sentinel.deferredRuntimeFnName");
...
export function assertNoUnresolvedDeferredRuntimeFunctions(
  ...
  if (unresolved.length > 0) {
    throw new Error(`[Sentinel] ${label} incomplete: ${unresolved.join(", ")}`);
  }
}
...
export function assertRuntimeFunctionsPresent(
  ...
  if (missing.length > 0) {
    throw new Error(`[Sentinel] ${label} missing functions: ${missing.join(", ")}`);
  }
}
...
export function assertRuntimeFunctionsResolved(
  ...
  if (missing.length > 0 || unresolved.length > 0) {
    ...
    throw new Error(`[Sentinel] ${label} ${parts.join(" | ")}`);
  }
}
