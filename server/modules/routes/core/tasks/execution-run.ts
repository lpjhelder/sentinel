...
export function registerTaskRunRoute(deps: TaskRunRouteDeps): void {
  ...
  app.post("/api/tasks/:id/run", (req, res) => {
    ...
    appendTaskLog(id, "system", `Git worktree created: ${worktreePath} (branch: claw-republic/${id.slice(0, 8)})`);
...
\`\`\``,
    ...
    const prompt = buildTaskExecutionPrompt(
      [
        ...
        `NOTE: You are working in an isolated Git worktree branch (claw-republic/${id.slice(0, 8)}). Commit your changes normally.`,
    ...
    if (provider === "api") {
      ...
      const worktreeNote = pickL(
        l(
          [` (격리 브랜치: claw-republic/${id.slice(0, 8)})`],
          [` (isolated branch: claw-republic/${id.slice(0, 8)})`],
          [` (分離ブランチ: claw-republic/${id.slice(0, 8)})`],
          [`（隔离分支: claw-republic/${id.slice(0, 8)}）`],
        ),
        taskLang,
      );
      notifyCeo(
