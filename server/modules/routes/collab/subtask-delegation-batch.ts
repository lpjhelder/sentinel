      ...
      if (["claude", "codex", "gemini", "opencode", "kimi", "copilot", "antigravity", "api"].includes(execProvider)) {
        ...
        try {
          ...
          appendTaskLog(
            delegatedTaskId,
            "system",
            `Git worktree created: ${worktreePath} (branch: sentinel/${delegatedTaskId.slice(0, 8)})`,
          );
          ...
          const worktreeNote = `\nNOTE: You are working in an isolated Git worktree branch (sentinel/${delegatedTaskId.slice(0, 8)}). Commit your changes normally.`;
          ...
          try {
            ...
            for (const sib of siblingRows) {
              ...
              const wtPath = path.join(projPath, ".sentinel-worktrees", shortId);
