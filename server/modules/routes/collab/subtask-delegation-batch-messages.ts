...
export function buildWorktreeCeoNote(
  ...
  return deps.pickL(
    deps.l(
      [` (격리 브랜치: sentinel/${delegatedTaskId.slice(0, 8)})`],
      [` (isolated branch: sentinel/${delegatedTaskId.slice(0, 8)})`],
      [` (分離ブランチ: sentinel/${delegatedTaskId.slice(0, 8)})`],
      [`（隔离分支: sentinel/${delegatedTaskId.slice(0, 8)}）`],
    ),
    lang,
  );
}
