    ...
    try {
      ...
      if (isTelegramConflictError(status.lastError)) {
        ...
        if (!conflictWarnedInPause) {
          console.warn(`[Sentinel] telegram receiver paused: ${status.lastError}`);
          console.warn(
            `[Sentinel] telegram receiver hint: another bot instance is polling this token. retry in ${Math.round(
        ...
        conflictWarnedInPause = false;
        console.warn(`[Sentinel] telegram receiver error: ${status.lastError}`);
      }
    } finally {
      busy = false;
