...
<p align="center">
  ...
  <a href="#opensentinel-integration">OpenClaw 集成</a> &middot;
  <a href="#direct-messenger-without-opensentinel">直连消息渠道</a> &middot;
...
Sentinel 将通过 **CLI**、**OAuth** 或 **直接 API Key** 连接的 AI 编程助手，转化为一个完整模拟的**虚拟软件公司**。您是 CEO，AI 代理是员工。看着它们跨部门协作、召开会议、完成任务、不断成长 —— 一切都通过迷人的像素风格办公室界面直观呈现。
...
- 文件直链: [`Sample_Img/sentinel-intro.mp4`](Sample_Img/sentinel-intro.mp4)
...
无需 OpenClaw，也可以仅用 Sentinel 直接配置并运行消息渠道。
...
- `OPENCLAW_CONFIG=<opensentinel.json 绝对路径>`（推荐不加引号）

`scripts/opensentinel-setup.sh` / `scripts/opensentinel-setup.ps1` 在 `INBOX_WEBHOOK_SECRET` 缺失时会自动生成。
...
# 6. 设置 AGENTS.md 编排规则（教 AI 代理成为 Sentinel 项目经理）
...
- 调用 Sentinel REST API 创建任务、分配代理、报告状态
...
`install.sh` / `install.ps1`（或 `scripts/opensentinel-setup.*`）会在可用时自动检测并写入 `OPENCLAW_CONFIG` 到 `.env`。
...
当聊天消息以 `$` 开头时，Sentinel 会将其作为 CEO 指令处理：
...
curl -X POST http://127.0.0.1:8790/api/inbox \
  -H "content-type: application/json" \
  -H "x-inbox-secret: $INBOX_WEBHOOK_SECRET" \
  -d '{"source":"telegram","author":"ceo","text":"$立即修复生产环境登录故障","skipPlannedMeeting":true,"project_context":"之前在做的 sentinel 项目"}'
```
...
| `OPENCLAW_CONFIG`                   | 使用 OpenClaw 时推荐         | 网关目标发现/聊天转发使用的 `opensentinel.json` 绝对路径                   |
| `DB_PATH`                           | 否                           | SQLite 数据库路径（默认：`./sentinel.sqlite`）                      |
...
| `UPDATE_CHECK_REPO`                 | 否                           | 更新检查使用的 GitHub 仓库标识（默认：`lpjhelder/sentinel`）  |
...
当 GitHub 发布了更高版本时，Sentinel 会在 UI 顶部显示包含 pull 操作说明和发布说明链接的横幅。
...
│   ├── opensentinel-setup.sh     # 一键安装（macOS/Linux）
│   ├── opensentinel-setup.ps1    # 一键安装（Windows PowerShell）
│   ├── prepare-e2e-runtime.mjs
│   ├── preflight-public.sh   # 发布前安全检查
│   └── generate-architecture-report.mjs
├── install.sh                # scripts/opensentinel-setup.sh 包装脚本
├── install.ps1               # scripts/opensentinel-setup.ps1 包装脚本
├── docs/                     # 设计与架构文档
├── AGENTS.md                 # 本地代理/编排规则
