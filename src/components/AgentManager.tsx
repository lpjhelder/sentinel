  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 sm:space-y-5">
      <div className="flex items-center justify-end gap-2">
        ...
        >
          + {tr("부서 추가", "Add Dept", "部署追加", "添加部门", "Add Depto")}
        </button>
        <button
          onClick={openCreate}
        ...
        >
          + {tr("신규 채용", "Hire Agent", "新規採用", "招聘", "Contratar Agente")}
        </button>
      </div>
      ...
      >
        {[
          {
            key: "agents" as const,
            label: tr("직원관리", "Agents", "社員管理", "员工管理", "Agentes"),
            icon: <StackedSpriteIcon sprites={randomIconSprites.tab} />,
          },
          { key: "departments" as const, label: tr("부서관리", "Departments", "部署管理", "部门管理", "Departamentos"), icon: "🏢" },
        ].map((tab) => (
          <button
            key={tab.key}
