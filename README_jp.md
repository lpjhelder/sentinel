...
<p align="center">
  ...
  <a href="#opensentinel-integration">OpenClaw連携</a> &middot;
  <a href="#direct-messenger-without-opensentinel">直接メッセンジャー</a> &middot;
...
Sentinelは **CLI**、**OAuth**、**直接APIキー** で接続されたAIコーディングアシスタントを、完全にシミュレートされた**仮想ソフトウェア会社**へと変革します。あなたがCEOです。AIエージェントたちが社員として働きます。部署をまたいで協力し、会議を開き、タスクを完遂し、レベルアップしていく様子を、魅力的なピクセルアートのオフィスインターフェースを通じてリアルタイムに観察できます。
...
- ファイルリンク: [`Sample_Img/sentinel-intro.mp4`](Sample_Img/sentinel-intro.mp4)
...
OpenClawを使わなくても、Sentinel単体でメッセンジャーチャネルを直接運用できます。
...
- `OPENCLAW_CONFIG=<opensentinel.json の絶対パス>`（引用符なし推奨）

`scripts/opensentinel-setup.sh` / `scripts/opensentinel-setup.ps1` は `INBOX_WEBHOOK_SECRET` が未設定なら自動生成します。
...
# 6. AGENTS.mdオーケストレーション規則をセットアップ（AIエージェントにSentinelプロジェクトマネージャーの役割を付与）
...
- Sentinel REST APIを呼び出してタスク作成、エージェント割り当て、ステータス報告
...
`install.sh` / `install.ps1`（または `scripts/opensentinel-setup.*`）は、可能な場合に `OPENCLAW_CONFIG` を自動検出して `.env` に設定します。
...
チャットメッセージが `$` で始まる場合、SentinelはCEOディレクティブとして扱います。
...
curl -X POST http://127.0.0.1:8790/api/inbox \
  -H "content-type: application/json" \
  -H "x-inbox-secret: $INBOX_WEBHOOK_SECRET" \
  -d '{"source":"telegram","author":"ceo","text":"$本番ログイン不具合を今すぐホットフィックス","skipPlannedMeeting":true,"project_context":"既存の sentinel プロジェクト"}'
```
...
| `OPENCLAW_CONFIG`                   | OpenClaw利用時推奨          | ゲートウェイターゲット検出/チャット連携で使う `opensentinel.json` の絶対パス                      |
| `DB_PATH`                           | 任意                        | SQLiteデータベースパス（デフォルト: `./sentinel.sqlite`）                                  |
...
| `UPDATE_CHECK_REPO`                 | 任意                        | 更新確認に使う GitHub リポジトリスラッグ（デフォルト: `lpjhelder/sentinel`）         |
...
GitHub により新しいリリースが公開されると、Sentinel は UI 上部に pull 手順とリリースノートリンク付きバナーを表示します。
...
Sentinelは次の3種類のプロバイダー接続に対応しています：
...
│   ├── opensentinel-setup.sh     # ワンクリックセットアップ（macOS/Linux）
│   ├── opensentinel-setup.ps1    # ワンクリックセットアップ（Windows PowerShell）
│   ├── prepare-e2e-runtime.mjs
│   ├── preflight-public.sh   # リリース前セキュリティチェック
│   └── generate-architecture-report.mjs
├── install.sh                # scripts/opensentinel-setup.sh のラッパー
├── install.ps1               # scripts/opensentinel-setup.ps1 のラッパー
